...
export default function Hero() {
  ...
  useEffect(() => {
    async function checkMembershipStatus() {
      ...
      try {
        ...
        console.log("🔍 DB response raw string check:", data?.membership_status);

        // Trim any hidden whitespace and force lowercase comparison safely
        const status = data?.membership_status?.trim().toLowerCase();
        
        if (status === "active") {
          console.log("✅ Setting isActiveMember to TRUE");
          setIsActiveMember(true);
        } else {
          console.log("❌ Setting isActiveMember to FALSE");
          setIsActiveMember(false);
        }
      } catch (err) {
  ...
  return (
    <section className={`relative min-h-[90vh] w-full overflow-hidden bg-[#050505] flex items-center ${greatVibes.variable} ${playfair.variable}`}>
