...
export default function EventDetailPage({
  ...
  if (loading)
    return (
      <div className="text-center text-amber-500 mt-20 animate-pulse font-medium tracking-wide">
        Loading showcase information...
      </div>
    );
  if (!event)
    return (
      <div className="text-center text-red-400 mt-20 font-medium">
        Showcase timeline could not be parsed.
      </div>
    );
          ...
          <div className="p-6 md:p-8 relative">
            ...
            <div className="mb-8">
              ...
              <p className="text-gray-400 text-sm leading-relaxed whitespace-pre-wrap">
                {event.description || "No description published for this showcase."}
              </p>
            </div>
              ...
              {event.artists && event.artists.length > 0 ? (
                <div className="grid gap-4 sm:grid-cols-2">
                  {event.artists.map((artist) => (
                    <div
                      key={artist.id}
                      className="flex gap-4 p-4 rounded-xl bg-[#0B0C10]/40 border border-white/5 items-center"
                    >
                      <img
                        src={
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-amber-500 uppercase tracking-wider font-medium mt-0.5">
            ...
            {showingPendingRequest && (
              <div className="mb-6 flex items-center gap-3 rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3">
                ...
                <p className="text-sm text-amber-300">
                  Your payment screenshot has been submitted and is awaiting review. We'll email you once your
                  ticket is confirmed.
                </p>
              </div>
            )}
            {showingRejectedRequest && (
              <div className="mb-6 flex items-center gap-3 rounded-xl border border-red-500/20 bg-red-500/5 px-4 py-3">
                ...
                <p className="text-sm text-red-300">
                  ...
                  {registrationRequestNote ? ` Note: ${registrationRequestNote}` : ""} Please try again with a
                  clear screenshot.
                </p>
              </div>
            )}
            ...
            {!event?.isMember && (
              <div className="mt-5 rounded-xl border border-amber-500/20 bg-amber-500/10 p-4">
                <p className="text-xs leading-6 text-amber-100">
                  ...
                  Payments for event tickets are securely processed through Razorpay by our parent organization.
                  During checkout or on your bank statement, you may see{" "}
                  <strong>Changing Minds Counseling & Psychotherapy Centre</strong> as the merchant name.
                </p>
              </div>
            )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4 overflow-y-auto py-8">
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#1f232d] p-6 shadow-2xl">
            ...
            <p className="text-sm text-gray-400 mb-4">
              ...
              <span className="text-amber-400 font-bold">₹{event.ticket_price}</span>. Then upload a screenshot of
              your payment confirmation — we'll review and confirm your ticket shortly.
            </p>
